# 🤖 Baileys WhatsApp API - By barmodss Official 
...
```sh
npm install @barmodssWorkPCodeNew/Baileys
```

Atau dengan **Yarn**:

```sh
yarn add @barmodssWorkPCodeNew/Baileys
```
...
```javascript
import makeWASocket, { DisconnectReason } from '@barmodssWorkPCodeNew/Baileys'
import { Boom } from '@hapi/boom'

async function connectToWhatsApp () {
    const barmodss = makeWASocket({
        // can provide additional config here
        printQRInTerminal: true
    })
    barmodss.ev.on('connection.update', (update) => {
    ...
    barmodss.ev.on('messages.upsert', m => {
        console.log(JSON.stringify(m, undefined, 2))

        console.log('replying to', m.messages[0].key.remoteJid)
        await barmodss.sendMessage(m.messages[0].key.remoteJid!, { text: 'Hello there!' })
    })
}
// run in main file
connectToWhatsApp()
...
```ts
const { generateWAMessageFromContent, proto } = require("@barmodssWorkPCodeNew/Baileys")
...
return barmodss.relayMessage(msg.key.remoteJid, msg.message, { messageId: msg.key.id })>
```


### 🏓 Mengirim Pesan Button  
```ts
barmodss.sendMessage(m.chat, {
     text: "Hello World !",
     footer: "JustKagenou",
     buttons: [ 
         { buttonId: `.play`,
...
### 📢 Mengirim Pesan Toko  
```ts
barmodss.sendMessage(msg.key.remoteJid, {
    text: "Isi Pesan",
    title: "Judul",
    subtitle: "Subjudul",
    footer: "Footer",
...
```ts
await barmodss.sendMessage(msg.key.remoteJid, {
    pollResult: {
        name: "Judul Polling",
        votes: [
            ["Opsi 1", 10], 
...
### 🏷️ Mention di Status  
```ts
await barmodss.StatusMentions({ text: "Halo!" }, [
...
### 📷 Pesan Album  
```ts
await barmodss.sendAlbumMessage(msg.key.remoteJid, [
...
```ts
await barmodss.sendMessage(msg.key.remoteJid, { keep: message.key, type: 1, time: 86400 })
await barmodss.sendMessage(msg.key.remoteJid, { pin: message.key, type: 1, time: 86400 })
```
...
💡 Punya ide keren? Silakan buat **Issue** di [repository ini](https://github.com/Im-notDev-barmodss/barmodssWorkPCodeNew-Baileys/issues).  

---

## 📬 Kontak

📩 **WhatsApp**: 082396674917 
🌍 **Website**: [Baileys API](https://github.com/Im-notDev-barmodss/barmodssWorkPCodeNew-Baileys)  

---

🚀 **Semoga kamu suka & semangat ngoding!** 🎉
# Copyright WhiskeyShocket/Baileys 
